<template>
  <NCard title="全局模型厂商" :bordered="false" class="catalog">
    <p class="hint">管理员配置密钥，用户使用已批准的模型。同步新模型后需单独批准，已有批准设置会保留。</p>
    ...
    <NForm label-placement="top" class="provider-form">
      ...
      <NFormItem label="模型列表接口（可选）"><NInput v-model:value="form.modelsUrl" :disabled="busy" placeholder="https://服务地址/v1/models；不支持时可手动添加" /></NFormItem>
    ...
    <NSpace><NButton type="primary" :loading="busy" @click="saveProvider">保存厂商</NButton><NButton v-if="selectedId" :disabled="busy" @click="sync">同步模型列表</NButton></NSpace>
    <template v-if="selectedId">
      <div class="manual-model"><NInput v-model:value="newModel" :disabled="busy" placeholder="手动添加模型 ID，例如 glm-5.3" @keydown.enter="addModel" /><NButton :disabled="busy || !newModel.trim()" @click="addModel">添加待批准模型</NButton></div>
      <NEmpty v-if="!models.length" description="尚无模型，请同步列表或手动添加" />
      <div v-else class="model-table">
        <div class="model-row model-heading"><span>模型 ID</span><span>允许用户使用</span><span>支持图片</span><span></span></div>
        <div v-for="model in models" :key="model.id" class="model-row">
          ...
          <NButton size="small" :disabled="busy" @click="saveModel(model)">保存</NButton>
        </div>
      </div>
      <p class="hint">图片能力需按厂商说明设置；模型名称不能证明能力。保存批准设置后生效，停用厂商会阻止其所有模型的新请求。</p>
    </template>
  </NCard>
</template>
...
import { listSharedProviders, saveSharedProvider, listSharedModels, syncSharedModels, saveSharedModel, type SharedProvider, type SharedModel } from '@/api/modelCatalog';
...
async function saveProvider() {
  await run(async () => { const { data: id } = await saveSharedProvider(selectedId.value, { ...form, apiKey: form.apiKey || undefined }); selectedId.value = id; form.apiKey = ''; providers.value = (await listSharedProviders()).data; models.value = (await listSharedModels(id)).data; message.success('厂商配置已保存'); });
}
async function sync() { const id = selectedId.value; if (!id) return; await run(async () => { models.value = (await syncSharedModels(id)).data; message.success('列表已同步，新模型需批准后才能使用'); }); }
async function addModel() { const id = selectedId.value; if (!id || !newModel.value.trim()) return; await run(async () => {
  ...
  await saveSharedModel(id, { modelName: newModel.value.trim(), approved: false, supportsVision: false }); models.value = (await listSharedModels(id)).data; newModel.value = '';
}); }
async function saveModel(model: SharedModel) { const id = selectedId.value; if (!id) return; await run(async () => { await saveSharedModel(id, model); models.value = (await listSharedModels(id)).data; message.success('模型权限已保存'); }); }
...
.model-row { display: grid; grid-template-columns: minmax(200px, 1fr) 120px 100px 64px; align-items: center; gap: 12px; padding: 12px 0; border-bottom: 1px solid var(--pa-line, #e2e8f0); min-width: 540px; }
