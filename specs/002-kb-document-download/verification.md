...
## 用户批准：知识库可见性筛选（2026-09-14）

在本地 codex/issue-185-react-optimization 按用户批准的 FR-010 实现全部/仅私人/仅公开。按 isPublic 分类，与文件名取交集；默认全部，按当前用户在浏览器保存选择；账号切换同步恢复，未知值或存储不可用回退全部。空结果可同时清除两种筛选。正在预览的资料被过滤掉时关闭预览；原统计和操作权限保持。
...
19:23 完成 22 tests passed（下载 7、页面 9、筛选 6），0 failed；TypeScript/生产构建通过，既有 bundle-size 警告仍在。git diff --check 通过。新增测试覆盖私有/本人公开/管理员公开分类、文件名交集、空结果、用户偏好恢复与隔离、登录/退出、非法存储值、存储失败与清除偏好。

Converge：FR-010 与 T020–T022 对应完整，无新增未完成实现项。此次只改前端与 SDD，未改后端、接口权限、上传/下载/预览授权或 LLM/RAG，因此未重复 Java/Engine/RAG 套件。未进行真实账号的浏览器验收；CSS 窄屏换行未经实际视口验收。偏好仅限当前浏览器，不承诺跨设备同步。

本次筛选无需重启后端；开发前端刷新/热更新，生产重建发布。回滚本次提交恢复原文件名筛选，已有资料和存储不变。交付时发现 PR #220 已于 2026-09-14 合并，当前分支已同步 main（8cd00ae1）。因此按用户授权提交推送原 185 分支，为本次改动新建 Draft PR，不合并；保留此前功能尚未执行的验收事项。
