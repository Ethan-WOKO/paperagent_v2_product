...
- 用户后续明确批准页面可见性筛选：全部（默认）、仅私人（isPublic=false）、仅公开（isPublic=true，包括本人公开和管理员公开）；与文件名筛选叠加，按用户记住本浏览器选择，空结果可清除全部筛选。仅前端展示过滤，后端/RAG 不变。
...
- 列表加法兼容、去重；他人的管理员公开文档展示基础元数据、解析文本预览和下载操作。删除与 DEMO_SEED 限制保持。
...
- 每次下载和共享预览重新鉴权，缺失/越权统一不暴露资源；存储故障安全失败，不暴露 objectKey 或存储地址。
