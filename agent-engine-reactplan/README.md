...
上下文压缩默认关闭，可在 **TS 引擎进程环境**中设置 `REACTPLAN_COMPACT_CONTEXT=true` 开启。入口不自动读取根 `.env`，当前 Compose 未透传此开关。2026-09-26 已移除批量 schema 加载入口及其环境开关；模型仍可在同一回复中发出多个单工具加载调用。仅对已持久化的历史待完成请求保留批量恢复兼容，之后的新请求统一使用单工具接口。
