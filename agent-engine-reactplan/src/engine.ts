import { runReactGraph, type ReactDecision } from "./react-graph.js";
import { projectModelContext } from "./model-context.js";
...
const LEGACY_LOAD_TOOL = functionTool(
  "load_tool",
...
export class AgentEngine {
  ...
  private async run(task: PersistedTask, signal: AbortSignal): Promise<void> {
    try {
      ...
      await runReactGraph({
        tools: () => this.executePendingCalls(task, signal),
        model: () => this.modelStep(task, signal)
      });
    } catch (error) {
  ...
  private async executePendingCalls(task: PersistedTask, signal: AbortSignal): Promise<ReactDecision> {
    if (terminal(task.view.state) || task.view.state === "waiting_user") return "stop";
  ...
  private async modelStep(task: PersistedTask, signal: AbortSignal): Promise<ReactDecision> {
      ...
      task.messages.push({
        ...
        content: JSON.stringify({
          ...
          tools: matches.map((tool) => ({
            ...
            description: bounded(tool.function.description.split(". ")[0]!, 180)
          })),
    ...
    if (call.name === "load_tool") {
      const names = args.names === undefined ? [requireString(args.name, "name")]
        : Array.isArray(args.names) && args.names.length >= 1 && args.names.length <= 6
          ...
          ? stableUnique(args.names as string[]) : [];
      if (!names.length) throw new EngineProblem(502, problem("MODEL_TOOL_ARGUMENTS_INVALID", "model", "load_tool requires name or one to six names"));
      for (const name of names) {
        if (!availableToolSpecs(task).some(tool => tool.function.name === name)) {
          throw new EngineProblem(502, problem("MODEL_TOOL_UNKNOWN", "model", "The requested tool is not present in the available tool catalog"));
        }
      ...
      task.loadedToolNames = stableUnique([...(task.loadedToolNames ?? []), ...names]);
      task.messages.push({ role: "tool", toolCallId: modelCallId(call), content: JSON.stringify({
        loaded: args.names === undefined ? names[0] : names,
        instruction: "These parameter schemas are now available. Call the tools only when needed."
      }) });
      return false;
    }
