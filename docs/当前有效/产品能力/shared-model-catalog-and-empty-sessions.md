...
1. 填写厂商名称、OpenAI Chat Completions 兼容地址、API Key，可选模型列表接口，并保存。支持 Base URL（自动补 `/chat/completions`）和完整聊天接口。列表接口必须填写完整地址。
2. “同步模型列表”读取厂商 `data[].id`；不支持列表接口时可以手动添加模型 ID。网络失败、HTTP 错误、空列表、格式错误不会清空原目录。
3. 同步得到的新模型默认未批准、未启用图片。管理员逐项设置“允许用户使用”“支持图片”并保存，厂商开启后才对用户可见。
...
仅支持当前已接入的 OpenAI Chat Completions 兼容协议；增加同协议模型无需再改代码。全新的接口协议仍需适配器。没有自动批准、定时同步或静默切换用户默认厂商。
