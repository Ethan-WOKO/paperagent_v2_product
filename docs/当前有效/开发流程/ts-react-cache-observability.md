...
日期：2026-09-25。分类：开发流程。关联 #185 / Draft PR #235。

## 当前合同

按用户确认，保留 LangGraph 调度与已有故障修复，将上下文压缩、批量工具加载改成可关闭的实验项且默认关闭；先建立测量依据，再决定是否推广。继续使用本地 185 checkout，不切换分支、不合并、不处理 #228，不修改其他对话的工作。
...
- `REACTPLAN_COMPACT_CONTEXT` 和 `REACTPLAN_BATCH_TOOL_LOADING` 仅在值为 `true` 时开启。缺省均关闭；新任务在检查点冻结开关，响应生成的待执行工具调用也冻结批量加载许可。关闭时模型不收到 names schema，运行时也拒绝未获许可的批量参数。
...
- `compact-v2` 才启用原有正文投影省略，仍可能破坏前缀并引起额外读取。批量加载仍只有单次最多六项限制，没有新引入累计 schema 预算；这是未推广的实验。
...
使用同样模型、任务、权限、工具目录和起始版本，分别记录默认、仅压缩、仅批量、两者开启。同时记录任务成功率、实际费用/缓存覆盖、总耗时、模型轮次、重复读取和沙箱耗时。冷/热缓存分别测量，避免固定实验顺序产生偏差。未经实际结果支持，不宣称之前 28% 字符减少对应费用节省，也不默认推广压缩或批量加载。
