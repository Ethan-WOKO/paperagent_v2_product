CREATE TABLE agent_v2_chain_transitions (
    ...
    CONSTRAINT ck_chain_transition_target_hash
        CHECK (target_identity_digest REGEXP '^[0-9a-f]{64}$')
);
...
CREATE TABLE agent_v2_chain_transition_stages (
    ...
    predecessor_authority_type VARCHAR(64) NULL,
    predecessor_authority_ref VARCHAR(128) NULL,
    successor_authority_type VARCHAR(64) NULL,
    successor_authority_ref VARCHAR(128) NULL,
...
CREATE TABLE agent_v2_chain_route_decisions (
    ...
    direct_task_spec_format_version INT NULL,
    direct_task_spec_sha256 CHAR(64) NULL,
    direct_task_spec_json LONGTEXT NULL,
    user_constraints_format_version INT NULL,
    user_constraints_sha256 CHAR(64) NULL,
    user_constraints_json LONGTEXT NULL,
    answer_required_refs_format_version INT NULL,
    answer_required_refs_sha256 CHAR(64) NULL,
    answer_required_refs_json LONGTEXT NULL,
    ...
    parent_route_decision_id VARCHAR(128) NULL,
    escalation_reason VARCHAR(2000) NULL,
    ...
    CONSTRAINT ck_chain_route_direct_shape CHECK (
        ...
        (needs_tool = 0 AND needs_network = 0 AND needs_project = 0
            ...
            AND direct_task_spec_sha256 REGEXP '^[0-9a-f]{64}$'
            ...
            AND user_constraints_sha256 REGEXP '^[0-9a-f]{64}$'
            ...
            AND answer_required_refs_sha256 REGEXP '^[0-9a-f]{64}$'
...
CREATE TABLE agent_v2_chain_instruction_dispositions (
    ...
    non_authoritative_reuse_suggestions_json LONGTEXT NOT NULL,
    created_at DATETIME(6) NOT NULL,
    PRIMARY KEY (disposition_id),
    CONSTRAINT uk_chain_disposition_event UNIQUE (event_id),
    CONSTRAINT uk_chain_disposition_task_identity
        UNIQUE (task_id, disposition_id),
...
CREATE TABLE agent_v2_chain_plan_bindings (
    ...
    CONSTRAINT ck_chain_plan_binding_hash
        CHECK (authority_sha256 REGEXP '^[0-9a-f]{64}$')
);
...
CREATE TABLE agent_v2_chain_candidate_step_results (
    ...
    CONSTRAINT ck_chain_candidate_result_candidate CHECK (
        ...
        (artifact_id IS NOT NULL
            ...
            AND diff_digest REGEXP '^[0-9a-f]{64}$')),
    ...
    CONSTRAINT ck_chain_candidate_result_hashes CHECK (
        receipt_refs_sha256 REGEXP '^[0-9a-f]{64}$'
        AND evidence_refs_sha256 REGEXP '^[0-9a-f]{64}$'
...
CREATE TABLE agent_v2_chain_review_decisions (
    ...
    fact_refs_json LONGTEXT NOT NULL,
    version_fence_sha256 CHAR(64) NOT NULL,
    created_at DATETIME(6) NOT NULL,
    ...
    CONSTRAINT ck_chain_review_hashes CHECK (
        fact_refs_sha256 REGEXP '^[0-9a-f]{64}$'
        AND version_fence_sha256 REGEXP '^[0-9a-f]{64}$')
);

CREATE TABLE agent_v2_chain_accepted_results (
    accepted_result_id VARCHAR(128)
    ...
    CONSTRAINT ck_chain_accepted_identity_hash
        CHECK (accepted_identity_sha256 REGEXP '^[0-9a-f]{64}$')
);
...
CREATE TABLE agent_v2_chain_result_applicability (
    ...
    source_decision_id VARCHAR(128)
        CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
    target_task_frame_id VARCHAR(128)
        CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
    target_plan_id VARCHAR(128)
        CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
    target_plan_revision_id VARCHAR(128)
        CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
    target_candidate_key VARCHAR(128)
        CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
    target_instruction_version_id VARCHAR(128)
        CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
...
CREATE TABLE agent_v2_chain_pending_items (
    ...
    missing_fields_json LONGTEXT NOT NULL,
    permission_scope VARCHAR(255) NULL,
    ...
    resume_position_json LONGTEXT NOT NULL,
    version_fence_sha256 CHAR(64) NOT NULL,
    created_at DATETIME(6) NOT NULL,
    PRIMARY KEY (gap_id),
    ...
    CONSTRAINT ck_chain_pending_hashes CHECK (
        gap_identity_sha256 REGEXP '^[0-9a-f]{64}$'
        AND missing_fields_sha256 REGEXP '^[0-9a-f]{64}$'
        AND resume_position_sha256 REGEXP '^[0-9a-f]{64}$'
        AND version_fence_sha256 REGEXP '^[0-9a-f]{64}$')
);
...
CREATE TABLE agent_v2_chain_pending_item_events (
    ...
    task_id VARCHAR(128) NOT NULL,
    event_id VARCHAR(128) NOT NULL,
    answer_instruction_id VARCHAR(128) NULL,
    validation_invocation_id VARCHAR(128) NULL,
    gap_validation_outcome VARCHAR(32) NULL,
    ...
    CONSTRAINT ck_chain_pending_item_detail CHECK (
        ...
        AND detail_sha256 REGEXP '^[0-9a-f]{64}$')
);
...
CREATE TABLE agent_v2_chain_action_bindings (
    ...
    effect_intent_id VARCHAR(128) NULL,
    dispatch_ref VARCHAR(128) NULL,
    result_authority_type VARCHAR(64) NULL,
    result_authority_ref VARCHAR(128) NULL,
    version_fence_sha256 CHAR(64) NOT NULL,
    created_at DATETIME(6) NOT NULL,
    ...
    CONSTRAINT ck_chain_action_signature
        CHECK (action_signature_sha256 REGEXP '^[0-9a-f]{64}$'),
    ...
    CONSTRAINT ck_chain_action_version_fence
        CHECK (version_fence_sha256 REGEXP '^[0-9a-f]{64}$')
);
...
CREATE TABLE agent_v2_chain_workspace_candidates (
    ...
    CONSTRAINT ck_chain_workspace_candidate_hashes CHECK (
        candidate_fingerprint REGEXP '^[0-9a-f]{64}$'
        AND diff_digest REGEXP '^[0-9a-f]{64}$'
        AND version_fence_sha256 REGEXP '^[0-9a-f]{64}$')
);
