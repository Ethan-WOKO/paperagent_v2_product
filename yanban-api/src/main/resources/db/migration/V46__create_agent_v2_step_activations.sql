CREATE TABLE agent_v2_step_activations (
    ...
    INDEX idx_agent_v2_step_activations_plan (plan_id),
    CONSTRAINT fk_agent_v2_step_activations_bootstrap
        FOREIGN KEY (plan_id) REFERENCES agent_v2_plan_bootstraps(plan_id),
    CONSTRAINT ck_agent_v2_step_activations_source
        CHECK (source_revision_number > 0
          AND source_checkpoint_version = 2
          AND source_event_sequence = 1),
    CONSTRAINT ck_agent_v2_step_activations_result
    ...
    CONSTRAINT ck_agent_v2_step_activations_owner
        CHECK (CHAR_LENGTH(TRIM(lease_owner_id)) > 0),
    CONSTRAINT ck_agent_v2_step_activations_fence CHECK (fencing_token > 0),
    CONSTRAINT ck_agent_v2_step_activations_formats
        CHECK (request_format_version = 1 AND result_format_version = 1),
    CONSTRAINT ck_agent_v2_step_activations_hashes
