CREATE TABLE agent_v2_chain_commands (
    ...
    target_client_request_id VARCHAR(128),
    gap_id VARCHAR(128),
    request_sha256 CHAR(64) NOT NULL,
    turn_id BIGINT,
    user_message_id BIGINT,
    result_task_id VARCHAR(128),
    result_event_id VARCHAR(128),
    result_instruction_id VARCHAR(128),
...
CREATE TABLE agent_v2_chain_tasks (
    ...
    project_id BIGINT,
    initial_project_version VARCHAR(255),
    ...
    CONSTRAINT ck_chain_task_root_hash
        CHECK (REGEXP_LIKE(root_request_sha256, '^[0-9a-f]{64}$')),
...
CREATE TABLE agent_v2_chain_authority_events (
    ...
    CONSTRAINT ck_chain_event_source_hash
        CHECK (REGEXP_LIKE(source_identity_sha256, '^[0-9a-f]{64}$'))
);
...
CREATE TABLE agent_v2_chain_instructions (
    ...
    parent_instruction_id VARCHAR(128),
    answered_gap_id VARCHAR(128),
    ...
    CONSTRAINT ck_chain_instruction_body CHECK (
        ...
        (relation_kind <> 'CANCEL' AND message_id IS NOT NULL
            ...
            AND REGEXP_LIKE(body_sha256, '^[0-9a-f]{64}$'))),
    CONSTRAINT ck_chain_instruction_boundary_hash
        CHECK (REGEXP_LIKE(effective_boundary_digest, '^[0-9a-f]{64}$'))
);
