CREATE TABLE agent_v2_chain_finalization_readiness (
    ...
    accepted_set_json CLOB NOT NULL,
    ...
    CONSTRAINT ck_chain_readiness_scope_hash
        CHECK (REGEXP_LIKE(readiness_scope_key, '^[0-9a-f]{64}$')),
    ...
    CONSTRAINT ck_chain_readiness_hashes CHECK (
        REGEXP_LIKE(accepted_set_sha256, '^[0-9a-f]{64}$')
        AND REGEXP_LIKE(coverage_sha256, '^[0-9a-f]{64}$')
        AND REGEXP_LIKE(publish_requirement_digest, '^[0-9a-f]{64}$')),
...
CREATE TABLE agent_v2_chain_finalization_checks (
    ...
    CONSTRAINT ck_chain_finalization_check_hashes CHECK (
        REGEXP_LIKE(accepted_set_sha256, '^[0-9a-f]{64}$')
        AND REGEXP_LIKE(publish_requirement_digest, '^[0-9a-f]{64}$')
        AND REGEXP_LIKE(input_digest, '^[0-9a-f]{64}$')
        AND REGEXP_LIKE(content_digest, '^[0-9a-f]{64}$')
        AND REGEXP_LIKE(publish_digest, '^[0-9a-f]{64}$')),
...
CREATE TABLE agent_v2_chain_task_outcomes (
    ...
    instruction_id VARCHAR(128) NOT NULL,
    task_frame_id VARCHAR(128),
    final_plan_id VARCHAR(128),
    ...
    CONSTRAINT ck_chain_task_outcome_hashes CHECK (
        REGEXP_LIKE(coverage_sha256, '^[0-9a-f]{64}$')
        AND REGEXP_LIKE(accepted_set_sha256, '^[0-9a-f]{64}$')
        AND REGEXP_LIKE(incomplete_items_sha256, '^[0-9a-f]{64}$')
        AND REGEXP_LIKE(limitations_sha256, '^[0-9a-f]{64}$')
        AND REGEXP_LIKE(risks_sha256, '^[0-9a-f]{64}$')),
...
CREATE TABLE agent_v2_chain_delivery_message_reservations (
    ...
    CONSTRAINT ck_chain_delivery_reservation_hash
        CHECK (REGEXP_LIKE(answer_body_sha256, '^[0-9a-f]{64}$')),
...
CREATE TABLE agent_v2_chain_deliveries (
    ...
    task_outcome_id VARCHAR(128),
    gap_id VARCHAR(128),
    decision_id VARCHAR(128),
    answer_content_id VARCHAR(128),
