CREATE TABLE agent_v2_chain_context_revisions (
    ...
    parent_context_revision_id VARCHAR(128),
    ...
    plan_id VARCHAR(128),
    plan_revision_id VARCHAR(128),
    plan_revision_number BIGINT,
    step_id VARCHAR(128),
    activation_event_id VARCHAR(128),
    project_id BIGINT,
    ...
    request_manifest_format_version INT,
    request_manifest_json CLOB,
    request_digest CHAR(64),
    completion_token VARCHAR(128),
    blocked_error_code VARCHAR(64),
    ...
    CONSTRAINT ck_chain_context_terminal_cut CHECK (
        ...
        (status = 'COMPLETE' AND module_count = 13
            ...
            AND REGEXP_LIKE(request_digest, '^[0-9a-f]{64}$')
...
CREATE TABLE agent_v2_chain_context_modules (
    ...
    projection_parameters_json CLOB NOT NULL,
    ...
    CONSTRAINT ck_chain_context_module_hashes CHECK (
        REGEXP_LIKE(source_version_sha256, '^[0-9a-f]{64}$')
        AND REGEXP_LIKE(read_boundary_sha256, '^[0-9a-f]{64}$')
        AND REGEXP_LIKE(projection_parameters_sha256, '^[0-9a-f]{64}$')
        AND REGEXP_LIKE(projection_digest, '^[0-9a-f]{64}$'))
);
...
CREATE TABLE agent_v2_chain_contents (
    ...
    CONSTRAINT ck_chain_content_hash
        CHECK (REGEXP_LIKE(body_sha256, '^[0-9a-f]{64}$'))
);
...
CREATE TABLE agent_v2_chain_model_proposals (
    ...
    body_authority_type VARCHAR(32),
    body_authority_ref VARCHAR(128),
    created_at TIMESTAMP(6) NOT NULL,
    ...
    CONSTRAINT ck_chain_proposal_hashes CHECK (
        ...
        AND REGEXP_LIKE(source_refs_sha256, '^[0-9a-f]{64}$')),
...
CREATE TABLE agent_v2_chain_proposal_state_events (
    ...
    official_authority_type VARCHAR(64),
    official_authority_ref VARCHAR(128),
